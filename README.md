...
- 已提供轻量多渠道启动向导 `npm run cli:serve`：当前可检查 Codex、引导微信登录、设置首个 route 绑定策略，并在首页确认后启动服务。
...
多渠道配置交互、本地运行状态持久化和渠道实例管理见 [docs/multi-channel-design.zh-CN.md](docs/multi-channel-design.zh-CN.md)。当前先落地轻量 `cli:serve` 启动向导，完整管理能力继续迭代。
...
当前 `cli:serve` 只启用微信真实渠道。配置阶段不会启动微信长轮询；只有回到首页选择“启动服务”后才进入常驻服务。完整渠道管理和持久化仍按多渠道设计文档继续迭代。
