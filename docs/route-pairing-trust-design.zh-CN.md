...
TUI 需要把 route 信任管理做成独立入口，不混入“聊天绑定”的主流程。原因是信任和绑定是两层不同状态：
...
待配对聊天
> 飞书 / 大龙虾 / 张三        最近活跃 2026-05-18 22:10
  微信 / default / 李四        最近活跃 2026-05-18 22:08

已信任聊天
  飞书 / 大龙虾 / 王五        配对 2026-05-18 21:30
  微信 / default / 主聊天      配对 2026-05-18 20:11

说明
配对码只会在运行日志里显示，不会发送到微信或飞书。
```
...
- `↑↓` 选择聊天 route。
- `Enter` 查看配对详情。
- `m` 本机手动信任选中的待配对 route。
- `r` 撤销选中的已信任 route，并保留 session 绑定。
- `u` 撤销选中的已信任 route，并解绑当前 session。
- `Esc/q` 返回。
...
聊天        飞书 / 大龙虾 / 王五
状态        已信任
信任时间    2026-05-18 21:30
信任方式    配对码
信任人      ou_xxx / 王五
当前绑定    session xxx

操作
1. 撤销信任，保留 session 绑定
2. 撤销信任，并解绑 session
3. 返回
```
...
- 待配对 route 可以显示，但不能在聊天绑定页绑定 session。
- 聊天绑定页需要标记“待配对”，并提示先完成配对。
- 手动信任只能由本机 TUI 触发，持久化为 `trustMethod: "manual"`。
- 撤销信任默认不解绑 session；撤销后该聊天再次发消息会重新要求配对，重新配对后可以继续使用原绑定。
- “撤销并解绑”是强清理操作，需要二次确认；它只释放 session 绑定，不删除 Codex session。
- 启动前 TUI 不显示配对码。配对码是运行时内存挑战，只在运行日志里显示，重启后失效。
- 待配对列表来自 `routes.json - trusted-routes.json`，即已经被发现过但还未信任的 route；没有发过消息的聊天不会出现在列表里。
