...
当前项目已经通过通用 `ChannelAdapter` 协议接入微信。多渠道阶段的目标是在不把 Bridge Core 重新写死到某个渠道的前提下，让微信、飞书、Telegram、Slack、终端等渠道可以在同一个中间件进程里同时工作。
...
这里的 route 指一个标准化聊天上下文，而不是单纯的渠道类型。例如一个微信私聊、一个微信群、一个飞书 thread、一个 Telegram chat 都是不同 route。
...
- 渠道类型：`weixin`、`lark`、`telegram`、`slack` 等实现类别。
- 渠道实例：运行时启动的一个 adapter 实例，例如 `weixin-main`、`lark-work`。多账号场景下，同一渠道类型可以有多个实例。
- 账号：渠道侧的机器人账号或登录账号，例如微信登录账号、飞书 bot、Telegram bot。
...
telegram-home:bot-1:direct:user-abc
```
...
- `thread`：支持 thread 的平台里的具体主题串，例如飞书 thread、Slack thread。`conversationId` 是 thread ID。
...
当前微信真实适配以私聊为主；微信消息协议里保留 `group_id` 映射到 `group` 的能力，但群聊链路需要后续真实验证。飞书、Slack 等后续渠道如果天然支持群聊和 thread，应按平台能力选择 `group` 或 `thread`，不要为了并发把同一个实际聊天上下文拆成多个 route。
...
这样做的目标是让微信的一对一限制、飞书的群聊/thread、Telegram 的 chat、Slack 的 channel/thread 都能接入同一个中间件协议，而不是把具体渠道差异写进 Bridge Core。
...
Terminal/Mock 使用默认完整投递策略。飞书、Slack、Telegram 等后续渠道应由各自 adapter 声明策略，例如卡片更新、thread reply、低频聚合或 suppress。新增渠道时优先扩展 `ChannelDeliveryPolicy`，不要在 Bridge Core 增加 `if channel === "xxx"`。
...
- 多渠道启动向导、渠道实例管理和 route/session 绑定管理还没有做。
...
4. 加载渠道实例列表。
5. 对每个渠道实例检查账号登录态。
6. 展示已知 route/session 绑定。
7. 选择未绑定 route 的默认策略。
8. 保存运行状态。
9. 回到首页，由用户确认后一键启动所有 enabled 且登录条件满足的渠道。
...
微信登录成功后回到渠道管理页，再由用户决定是否继续添加其他渠道。这样用户可以先准备多个渠道实例，再回首页一键启动。
...
微信登录显示二维码或二维码文本；飞书可显示授权 URL 或使用 bot token；Telegram/Slack 可提示检查 token 环境变量。不同渠道的登录方式由 adapter 处理，但 CLI 交互框架统一。
...
`/resume` 在微信、飞书、Telegram 或终端里行为完全一致：只能绑定当前 route 可拥有的 session；如果 session 已被另一个 route 拥有，必须拒绝。

## 13. 实施顺序

本轮只做核心多渠道内核，不做配置文件、启动向导和真实第二渠道。目标是让 Bridge Core 具备多 `ChannelAdapter` 接入能力，同时保持现有 `weixin codex`、`terminal codex` 单渠道入口兼容。
